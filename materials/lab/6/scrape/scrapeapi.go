...
// NOTICE: For some reason my code won't let me use a few of the functions needed like regexp, and mux. It says that they are undefined
// not sure whay I am doing incorrectly 
        ...
        for _, r := range regexes {
            if r.MatchString(path) {
				...
				if exists == false {
					...
					// Have to set it back to false so that it checks every new item
					exists = false;
				}
...
func MainPage(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Fprintf(w, "<html><body><H1>This API will list every hit that it gets and then add it to an array</H1></body>")
}
...
func FindFile(w http.ResponseWriter, r *http.Request) {
    ...
    if ok && len(q[0]) > 0 {
		...
		if Found == false {
			Fprintf("No matches exist for that File")
			// For whatever reason it doesn't like this. Same issue with regexp and mux, says that it is undefined and I can't figure out why
		}
